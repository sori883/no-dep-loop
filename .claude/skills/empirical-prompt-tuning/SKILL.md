...
> 位置づけ: 本 skill はループの自動工程には組み込まれない補助 skill。controller が skill / slash command / プロンプトを新規作成・大幅改訂したときに手動で起動する（`$loop-engineering` の振り返りで「能力拡張」が必要と判断されたとき等）。
...
0. **Iteration 0 — description と body の整合チェック**（静的、dispatch 不要）
   ...
   - 例: description「navigation / form filling / data extraction」と書いてあるが body は単一 CLI コマンドの ref のみ、のような乖離を検出
...
- `loop-engineering` — ループ全体の設計。本 skill はその「能力拡張（skill / プロンプト改訂）」の検証手段として使う
- `cross-review-gates` — 工程成果物の独立レビュー。本 skill はレビュー対象となる指示そのものの品質を上げる